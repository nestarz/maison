...
function createCodegenContext(ast, { mode = 'function', prefixIdentifiers = mode === 'module' || mode === 'cjs', sourceMap = false, filename = `template.vue.html`, scopeId = null, ssr = false }) {
  const context = {
      mode,
      prefixIdentifiers,
      sourceMap,
...
function generate(ast, options = {}) {
  ...
  const { mode, push, helper, prefixIdentifiers, indent, deindent, newline, scopeId, ssr } = context;
...
const DOMErrorMessages = {
  ...
  [61 /* X_V_SHOW_NO_EXPRESSION */]: `v-show is missing expression.`
};
...
const transformShow = (dir, node, context) => {
  const { exp, loc } = dir;
  if (!exp) {
      context.onError(createDOMCompilerError(61 /* X_V_SHOW_NO_EXPRESSION */, loc));
  }
  return {
      props: [],
